      ...
      <div className="bg-white p-8 rounded-[32px] w-full max-w-sm shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 mb-6">
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-lg font-black text-slate-900">₹{(stock.currentPrice || stock.buyPrice).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
          </div>
          <div className="flex justify-between items-center text-rose-600">
            <span className="text-[10px] font-black uppercase tracking-widest opacity-80">Estimated Proceeds</span>
            <span className="text-sm font-black">₹{proceeds.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
          </div>
        </div>

        <div className="space-y-4">
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Quantity to Sell</label>
